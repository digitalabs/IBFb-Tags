...
datos<-datos[order(datos$LOC,datos$ENTRY,datos$REP,datos$BLK),]
...
Rm1MLp <- lmer(pruebas[,5] ~ ENTRY + (1 |(REP):BLK),pruebas, REML =  TRUE, verbose = TRUE,na.action="na.omit")
...
prues<-datoss[,c(1:4,iter)]
pruebas<-prues[order(prues$LOC,prues$ENTRY,prues$REP,prues$BLK),]
Rm1MLp <- lmer(pruebas[,5] ~ 1+(1 | LOC) +  (1 |LOC:ENTRY)+ (1 | (REP:LOC):BLK),pruebas, REML = TRUE, verbose = TRUE,na.action="na.omit")
...
fm2<-lme(todos ~ ENTRY,pruebas, random =~ 1 |BLK,  method="REML",na.action = na.omit)
an<-anova(fm2)
