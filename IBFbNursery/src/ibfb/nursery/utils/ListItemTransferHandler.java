...
import ibfb.nursery.mainwizard.NurseryVisualPanel9;
import ibfb.nursery.core.NurseryEditorTopComponent;
import ibfb.nursery.mainwizard.NurseryVisualPanel10;
...
public class ListItemTransferHandler extends TransferHandler {
    ...
    private void cleanup(JComponent c, boolean remove) {
        if (remove && indices != null) {
            ...
            if (type == 0) {
                NurseryVisualPanel9.cleanFields();
            }

            if (type == 1) {
                NurseryVisualPanel10.cleanFields();
            }

            if (type == 2) {
                NurseryEditorTopComponent nurseryWindow = (NurseryEditorTopComponent) WindowManager.getDefault().getRegistry().getActivated();
